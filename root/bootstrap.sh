...
adminnewhash=$(sh -c "OPENSEARCH_JAVA_HOME=/usr/local/openjdk${jdk_v} bash /usr/local/lib/opensearch/plugins/opensearch-security/tools/hash.sh -p ${adminpass} | sed -e /^*/d")
kibananewhash=$(sh -c "OPENSEARCH_JAVA_HOME=/usr/local/openjdk${jdk_v} bash /usr/local/lib/opensearch/plugins/opensearch-security/tools/hash.sh -p ${kibanapass} | sed -e /^*/d")
